variable "template_description" {
  type    = string
  default = "Ubuntu 18.04 x86_64 template built with packer (${env("vm_ver")}). Username: ${env("default_vm_user")}"
}
...
source "proxmox" "instance" {
  ...
  boot_command = ["<esc><wait>", "<esc><wait>", "<enter><wait>", "/install/vmlinuz initrd=/install/initrd.gz", " auto=true priority=critical", " url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/preseed.cfg", "<enter>"]
  boot_wait    = "10s"
...
build {
  ...
  provisioner "ansible" {
    ...
    extra_arguments    = ["${var.ansible_verbosity}", "--extra-vars", "default_vm_user=${var.default_vm_user}", "--tags", "all,is_template", "--skip-tags", "openbsd,alpine"]
